...
for filepath in "${files[@]}"; do
    ...
    cp "${abbreviated}wLEDs_detections_raw.png" "/c/Users/trist/UtahPhotos/Detections"
    ...
    line_star=$(echo "$output" | grep "Star centroid xy")
    star_x=$(echo "$line_star" | sed -E 's/.*x=([-0-9.]+).*/\1/')
    star_y=$(echo "$line_star" | sed -E 's/.*y=([-0-9.]+).*/\1/')
    line_offset=$(echo "$output" | grep "dx=")
    dx_pix=$(echo "$line_offset" | sed -E 's/.*dx=([-0-9.]+).*/\1/')
    dy_pix=$(echo "$line_offset" | sed -E 's/.*dy=([-0-9.]+).*/\1/')


    printf "%-20s %8s %12.3f %12.3f\n" "$abbreviated" "[$count/$total]" "$dx_pix" "$dy_pix"
    #printf "%-20s %8s %12.3f %12.3f\n" "$abbreviated" "[$count/$total]" "$star_x" "$star_y"
    #printf "%-20s %8s %12.3f %12.3f\n" "$abbreviated" "[$count/$total]" "$dEl_deg" "$dEast_deg" 

    #Step 3: Outputs; copy before moving files
    mv "${abbreviated}catalog.txt" "$BATCH_DIR/"
    cp "${abbreviated}overlay.png" "/c/Users/trist/UtahPhotos/Nonzoom_Overlayed"
