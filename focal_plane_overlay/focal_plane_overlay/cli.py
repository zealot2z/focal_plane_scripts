...
def build_parser():
    ...
    #Added 6/22/26 for calibration square; standard vmax is 30 or 40 when using nonlog scale
    p.add_argument('--calibration-output', default=None,
               help='Squared calibration image')
    p.add_argument('--calibration-vmax', type=float, default=15,
...
def main():
    ...
    half_size = 67.5 #pixels; equal to half of Firstly square side length; Actual sidelength = 107 pixels, size of module (some stars lay outside this),
    #but for some reason still have an in-focus image. 
    ...
    if args.calibration_output:
        ...
        ##attempts to highlight lattice more
        img_log = np.log10(img8 + 1)
        #have tried np.log((args.calibration_vmax))
        #axz.imshow(img_log, origin='upper', cmap='gray', vmin=0, vmax=np.log((args.calibration_vmax)))

        axz.plot(cam_center[0], cam_center[1], marker='+', markersize=18,
                 markeredgewidth=2.5, color='blue') #default color = lime
        
        """
        #code to highlight lattice
        ...
        axz.imshow(img_step3, origin='upper', cmap='gray')
        """

        #actual code that draws square
        square_side_length = 108
        half_length = square_side_length/2
        square = Rectangle(
            (cam_center[0] - half_length, cam_center[1] - half_length),
            2 * half_length,
            2 * half_length,
            fill=False,
            edgecolor='blue',
