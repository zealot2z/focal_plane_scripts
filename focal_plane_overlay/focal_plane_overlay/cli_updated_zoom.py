...
def main():
    ...
    # had args.source_id
    src = cat.loc[cat['ID'] == int(src['ID'])].iloc[0]
...
#had args.source_id
    ...
    if args.zoom_output:
        ...
        axz.plot(cam_center[0], cam_center[1], marker='+', markersize=18,
                 markeredgewidth=2.5, color='lime')
        ...
        #had f'star ID {args.source_id}' ^^ NOT sure if this works; running cli.py not updated zoom so never had a chance to compile
        ...
        axz.annotate('', xy=(offset_start_xy[0] + d_el_pix_z[0], offset_start_xy[1] + d_el_pix_z[1]),
                     xytext=(offset_start_xy[0], offset_start_xy[1]),
                     arrowprops=dict(arrowstyle='->', lw=1.8, color='orange'))
        axz.text(offset_start_xy[0] + d_el_pix_z[0] + 5, offset_start_xy[1] + d_el_pix_z[1],
                 '+El', color='orange', fontsize=9)
        axz.annotate('', xy=(offset_start_xy[0] + d_e_pix_z[0], offset_start_xy[1] + d_e_pix_z[1]),
                     xytext=(offset_start_xy[0], offset_start_xy[1]),
                     arrowprops=dict(arrowstyle='->', lw=1.8, color='deepskyblue'))
        axz.text(offset_start_xy[0] + d_e_pix_z[0] + 5, offset_start_xy[1] + d_e_pix_z[1],
                 '+E', color='deepskyblue', fontsize=9)
